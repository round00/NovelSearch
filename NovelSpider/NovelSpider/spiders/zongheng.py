...
class ZonghengSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        #获取下页url
        next_url = ""
        next_page = response.css(".pagebox .pagenumber.pagebar a[title=下一页]::attr(page)").extract_first("")
        if len(next_page) != 0:
    ...
    def parse_detail(self, response):
        ...
        author = response.css(".book-side.fr .book-author .au-name a::text").extract_first("")
        # 类别标签
